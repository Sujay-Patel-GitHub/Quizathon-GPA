...
const adjustQuizDifficultyFlow = ai.defineFlow(
  ...
  async input => {
    // Return static adjustment to avoid calling AI when API key is missing
    const { studentPerformance, currentDifficulty } = input;
    let adjustedDifficulty = currentDifficulty;

    if (studentPerformance > 80) {
      if (currentDifficulty === 'Easy') adjustedDifficulty = 'Medium';
      else if (currentDifficulty === 'Medium') adjustedDifficulty = 'Hard';
    } else if (studentPerformance < 40) {
      if (currentDifficulty === 'Hard') adjustedDifficulty = 'Medium';
      else if (currentDifficulty === 'Medium') adjustedDifficulty = 'Easy';
    }

    return {
      adjustedDifficulty,
      reasoning: 'Performance-based difficulty adjustment (Rule-based)'
    };
  }
);
